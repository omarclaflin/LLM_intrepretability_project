...
def extract_hidden_states_from_start(model, tokenizer, texts, token_distance, target_layer=16):
    """
    Extract hidden states for windows from the start of each text.

    Args:
        model: Base language model
        tokenizer: Tokenizer
        texts: List of texts
        token_distance: Number of tokens to include from the start
        target_layer: Which model layer to extract hidden states from

    Returns:
        Array of shape (n_texts, token_distance, input_dim)
        and list of actual window sizes
    """
    ...
    # Check validity - much simpler now
    for i, text in enumerate(tqdm(texts, desc="Checking text lengths")):
        ...
        # Check if we have enough tokens for the window (need at least 1 token after special tokens)
        start_pos = 1 if tokenizer.bos_token else 0  # Skip BOS token if present
        if start_pos < seq_len:  # Ensure there's at least one content token
            valid_indices.append(i)
            # Calculate actual window size we'll get
            actual_window_size = min(token_distance, seq_len - start_pos)
            window_sizes.append(actual_window_size)
        else:
            logging.debug(f"Skipping text '{text[:50]}...' (ID: {i}) due to insufficient content tokens")

    logging.info(f"Found {len(valid_indices)} texts with sufficient content tokens")

    # Now extract hidden states for the valid indices
    for i, idx in enumerate(tqdm(valid_indices, desc="Extracting hidden states")):
        ...
        with torch.no_grad():
            ...
            # Extract window from start (skip BOS token if present)
            ...
            # Pad with zeros if needed to ensure consistent shape
    ...
    # Return hidden states and the actual window sizes for the texts that were processed
    return all_hidden_states, window_sizes
...
def run_individual_feature_rsa(set1_hidden_states, set2_hidden_states, sae_model, n_top_neurons=50, n_bootstraps=1000):
    ...
    # Collapse across token window for both sets (average across the window)
...
def main():
    ...
    parser.add_argument("--token_distance", type=int, default=5, help="Number of tokens to analyze from the start of each text")
    ...
    # Extract hidden states from the start of each statement
    logging.info(f"Extracting hidden states for '{set1_name}' statements...")
    set1_hidden_states, set1_window_sizes = extract_hidden_states_from_start(
    ...
    set2_hidden_states, set2_window_sizes = extract_hidden_states_from_start(
