...
def get_top_k_features(set1_activations, set2_activations, k=None, pos_index=None):
    """
    Get indices of top k features based on maximum activation values.

    Args:
        set1_activations: Activations from set 1
        set2_activations: Activations from set 2
        k: Number of top features to return (None for all)
        pos_index: Position index to focus on (None for middle position)

    Returns:
        Indices of top k features
    """
    if pos_index is None:
        # Use middle position by default (token_distance)
        pos_index = set1_activations.shape[1] // 2

    # Use activations at the specified position
    max_acts1 = np.max(set1_activations[:, pos_index, :], axis=0)
    max_acts2 = np.max(set2_activations[:, pos_index, :], axis=0)
    combined_max = np.maximum(max_acts1, max_acts2)

    if k is None or k >= len(combined_max):
        # Return all feature indices if k is None or too large
        return np.arange(len(combined_max))

    # Return indices of top k features
    return np.argsort(combined_max)[-k:]
...
def main():
    parser = argparse.ArgumentParser(description="Neuroscience analysis on SAE features")
    ...
    parser.add_argument("--input_json", type=str, required=True, help="JSON file with statements for two categories") # Updated help text
    ...
    parser.add_argument("--rsa_top_k", type=int, default=100, help="Number of top features to use for RSA analysis")
    ...
    output_dir = Path(args.output_dir) / f"neuroscience_analysis_{timestamp}"
    ...
    # Get top k features based on maximum activation at middle position
    middle_pos = args.token_distance  # Middle position in the window (0-indexed)
    # Ensure middle_pos is a valid index for the extracted activations
    if middle_pos >= set1_activations.shape[1] or middle_pos >= set2_activations.shape[1]:
         logging.error(f"Middle position index {middle_pos} is out of bounds for extracted activations (shape: {set1_activations.shape}). Cannot determine top features based on middle position.")
         # Fallback: Select top features based on overall max activation across the window
         logging.info("Falling back to selecting top features based on overall max activation across the window.")
         top_k_features = get_top_k_features(
             set1_activations.max(axis=1), set2_activations.max(axis=1), # Use max across positions
             k=args.top_k, pos_index=None # pos_index=None signals to use the whole array
         )
    else:
        top_k_features = get_top_k_features(
            set1_activations, set2_activations,
            k=args.top_k, pos_index=middle_pos
        )

    logging.info(f"Selected top {len(top_k_features)} features for analysis")

    # Run t-tests on top features
    logging.info("Running t-tests to compare activations between sets...")
    p_values, t_values = run_t_tests(set1_activations, set2_activations, top_k_features)

    # Plot p-value heatmap
    logging.info("Creating p-value heatmap...")
    plot_p_value_heatmap(
        p_values, t_values, top_k_features,
        output_dir / "p_value_heatmap.png",
        token_distance=args.token_distance,
        statements_dict=statements_dict, # Pass the statements dict
        category_names=category_names, # Pass the category names
        tokenizer=tokenizer
    )
    ...
    # Save top features and their statistics (t-values and p-values as requested)
    # Note: Z-scores are computed for plotting but not stored here as per original design.
    with open(output_dir / "top_features.json", "w") as f:
        json.dump(convert_numpy_types({
            ...
            # Z-scores are not stored here, only t and p values as in the original file
        }), f, indent=2)
    ...
    # Get top features for RSA (subset of the top_k_features)
    # Ensure rsa_top_k does not exceed the number of features found
    rsa_k = min(args.rsa_top_k, len(top_k_features))
    rsa_features = top_k_features[:rsa_k]
    logging.info(f"Using top {len(rsa_features)} features for RSA analysis")
