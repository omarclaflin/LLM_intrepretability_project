...
async def analyze_feature_meaning(
    ...
    logging.info(f"Loading wikitext dataset with {num_samples} samples...")
    try:
        dataset = load_dataset("wikitext", "wikitext-2-raw-v1", split="train")
        # Take a subset of texts for analysis
        # Ensure we take enough samples to potentially get num_samples non-empty, short texts
        initial_sample_size = min(len(dataset), num_samples * 2) # Take more initially
        texts = [sample["text"] for sample in dataset.select(range(initial_sample_size))]
        # Filter empty texts and texts that are too short after tokenization with max_length=100
        # We need texts that result in at least a few tokens after truncation
        filtered_texts = []
        for text in texts:
            if text.strip():
        ...
        texts = filtered_texts[:num_samples]

        logging.info(f"Loaded and filtered down to {len(texts)} text samples from wikitext (max_length=100)")
