...
def collect_activations(model, tokenizer, num_tokens):
    ...
    # dataset = load_dataset("hf-internal-testing/tiny-text", split="train", streaming=True)
    # Or: dataset = load_dataset("squad", split="train", streaming=True) # SQuAD is larger but common
    
    # Process data in batches
    for batch in tqdm(dataset.take(num_tokens // BATCH_SIZE), desc="Collecting activations"):
        # Tokenize batch
